...
def scans():
    ...
    if SCAN_TYPE=="Quick":
        ...
        # user = getUsername()
        AppdataPath = R"C:\Users\$USERNAME"
        tempPath = R"${TEMP}"
        ...
        # x = time.time()
        scanReport = XylentScanner.scanFolders(location=["C:\\Program Files"])
        # y = time.time()
        # print("--------------")
        ...
        desktop = os.path.expandvars(desktopPath)
        # x = time.time()
        scanReport = XylentScanner.scanFolders(location=["C:\\"])
        # y = time.time()
        # print("--------------")
        # print("Time taken:",y-x)
        return scanReport
...
def cleanJunk():
    ...
    windowsTempPath = r"C:\Windows\Temp"
